...
const createMeme = async (req, res) => {
  ...
  upload.single("image")(req, res, async (err) => {
    ...
    let { user_id, tag, coordinatesArr } = req.body;
    coordinatesArr = JSON.parse(coordinatesArr)
    ...
    try {
      ...
      for (const coord of coordinatesArr) {
        console.log(coord);
        await dbInstance.query(queries.addCoordinates(), [meme_id, coord[0], coord[1]]);
      }
...
const getAllMeme = async (req, res) => {
  ...
  try {
    const result = await dbInstance.query(queries.getAllMeme());
    logger.info(result.rows);
    res.send({ data: result.rows });
  } catch (error) {
    logger.error(error);
